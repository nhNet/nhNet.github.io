	...
	switch (profileEmail){						//check if account exists
		...
		case "patrickthomas.sch@kcpupils.org":
			if(profileEmail=="spbong999@gmail.com" || profileEmail=="patrickthomas.sch@kcpupils.org"){	//check if account is pro
				...
				document.querySelector('.welcome').innerHTML="<div style='text-align:right;'><h1 style='font-size:20px;'> Welcome, <img width='20px' height='20px' src='"+profileImg+"'>"+profileName+"!&nbsp;</h1></div><div style='font-size:10px; width:25px;height:15px;background:#FF5000;border:1px solid white;'>Pro</div>";
			}else{
				Ad();
				document.body.innerHTML='<div class="welcome"></div><div id="Ad"></div>';
